...
static void input_callback(void* userdata, short* stream, int len){
...
static void output_callback(void* userdata, short* stream, int len){
...
int main(int argc, const char **argv){
	...
	printf("Peer for a connection esablished.\r\n");
	...
	while(running){
		...
		// update both vbo in a single pass (this doesnt really depict proper use of texture co-ordinates or normals. 
		// texture and normals info isn't important with GL_LINE_STRIP like it is with GL_TRIANGLES though it can be 
		// used to give a nice shine with a light enabled
		for (int i=0;i<SAMPLES;i++){
			in_waveform_vbo[i]  = (vertex_tcnv) { {1, 1}, {0,1,1,1}, {0, 0, 1}, {(float)(((-SAMPLES/2)*100)+i*100), (float)( in_waveform_buffer[i])*0.1f, 0} };
			out_waveform_vbo[i] = (vertex_tcnv) { {1, 1}, {1,0,1,1}, {0, 0, 1}, {(float)(((-SAMPLES/2)*100)+i*100), (float)(out_waveform_buffer[i])*0.1f, 0} };
		}
		...
		glLoadIdentity();
		{
			...
			Perspective(75.0f, (float)WINDOW_WIDTH/WINDOW_HEIGHT, 0.1f, 10000.0f);
			glRotatef(0, 1, 0, 0);
			glRotatef(0, 0, 1, 0);
			glRotatef(0, 0, 0, 1);
			glTranslatef(0, 0, 0);
